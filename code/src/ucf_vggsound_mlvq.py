...
def main():
    ...
    Decoder = Semantic_Decoder(input_dim=choose_channel*2, class_num=16) 
    ...
    for epoch in range(start_epoch+1, args.n_epoch):
        ...
        if ((epoch + 1) % args.eval_freq == 0) or (epoch == args.n_epoch - 1):
            loss = validate_epoch(Encoder, Decoder, val_dataloader, criterion, criterion_event, epoch, args, indices0, indices1)
            logger.info("-----------------------------")
...
def train_epoch(Encoder, Decoder, train_dataloader, criterion, criterion_event, optimizer, epoch, total_step, args, indices0, indices1):
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            if (args.dataset_name == 'ucfv_vgga'):
                vq,vq_2,_,_ = Encoder.Video_VQ_Encoder(feat)
                
            elif (args.dataset_name == 'vgga_ucfv'):
                vq,vq_2,_,_ = Encoder.Audio_VQ_Encoder(feat)
            else:
                raise NotImplementedError
            vq = vq[:,:,indices0]
            vq_2 = vq_2[:,:,indices1]
            vq = torch.cat((vq, vq_2), dim=-1)
...
@torch.no_grad()
def validate_epoch(Encoder,Decoder,val_dataloader, criterion, criterion_event, epoch, args, indices0, indices1, eval_only=False):
