...
from model.main_model_2 import Semantic_Decoder_AVVP, AVT_VQVAE_Encoder, AVT_MLVQVAE_Encoder
...
def main():
    ...
    '''The multiplication by 2 is because H-DCID has two layers of alignment'''
    Decoder = Semantic_Decoder_AVVP(input_dim=choose_channel*2, class_num=13) 
    ...
    if model_resume is True:
        path_checkpoints = "/../../data/checkpoints/rqvae/warmstart3-MLVQ-AVT-Noshared-400-deep2-model-3.pt"
    ...
    indices0 = cal_criterion(Encoder.Cross_quantizer.embedding[0].cuda(), choose_channel, args.toc_max_num, args.toc_min_num)
    indices1 = cal_criterion(Encoder.Cross_quantizer.embedding[1].cuda(), choose_channel, args.toc_max_num, args.toc_min_num)
    ...
    for epoch in range(start_epoch+1, args.n_epoch):
        ...
        loss, total_step = train_epoch(Encoder, Decoder,ExpLogLoss_fn, train_dataloader, criterion, criterion_event,
                                       optimizer, epoch, total_step, args, indices0, indices1)
        logger.info(f"epoch: *******************************************{epoch}")

        if ((epoch + 1) % args.eval_freq == 0) or (epoch == args.n_epoch - 1):
            loss = validate_epoch(Encoder, Decoder, ExpLogLoss_fn, val_dataloader, criterion, criterion_event, epoch, args, indices0, indices1)
            logger.info("-----------------------------")
...
def train_epoch(Encoder, Decoder, ExpLogLoss_fn, train_dataloader, criterion, criterion_event, optimizer, epoch, total_step, args, indices0, indices1):
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        if (args.dataset_name == 'ave_avvp_va'):
            with torch.no_grad():
                video_vq, video_vq_2, _, _ = Encoder.Video_VQ_Encoder(visual_feat)
                video_vq = video_vq[:,:,indices0]
                video_vq_2 = video_vq_2[:,:,indices1]
                video_vq = torch.cat((video_vq, video_vq_2), dim=-1)
            ...
            with torch.no_grad():
                audio_vq, audio_vq_2, _, _ = Encoder.Audio_VQ_Encoder(audio_feat)
                audio_vq = audio_vq[:,:,indices0]
                audio_vq_2 = audio_vq_2[:,:,indices1]
                audio_vq = torch.cat((audio_vq, audio_vq_2), dim=-1)
...
@torch.no_grad()
def validate_epoch(Encoder,Decoder,ExpLogLoss_fn, val_dataloader, criterion, criterion_event, epoch, args, indices0, indices1, eval_only=False):
    ...
    for n_iter, batch_data in enumerate(val_dataloader):
        ...
        if (args.dataset_name == 'ave_avvp_va'):
            audio_vq, audio_vq_2, _, _ = Encoder.Audio_VQ_Encoder(feature)
            audio_vq = audio_vq[:,:,indices0]
            audio_vq_2 = audio_vq_2[:,:,indices1]
            audio_vq = torch.cat((audio_vq, audio_vq_2), dim=-1)
            ...
            video_vq, video_vq_2, _, _ = Encoder.Video_VQ_Encoder(feature)
            video_vq = video_vq[:,:,indices0]
            video_vq_2 = video_vq_2[:,:,indices1]
            video_vq = torch.cat((video_vq, video_vq_2), dim=-1)
    ...
    f1_score = 2.0*downstream_accuracy.avg*downstream_recall.avg/(downstream_accuracy.avg+downstream_recall.avg+1e-6)
