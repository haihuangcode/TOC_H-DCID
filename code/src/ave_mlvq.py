...
from model.main_model_2 import Semantic_Decoder,AVT_VQVAE_Encoder,AVT_MLVQVAE_Encoder
...
def main():
    ...
    Decoder = Semantic_Decoder(input_dim=choose_channel*2, class_num=28) 
    ...
    for epoch in range(start_epoch+1, args.n_epoch):
        ...
        if ((epoch + 1) % args.eval_freq == 0) or (epoch == args.n_epoch - 1):
            loss = validate_epoch(Encoder, Decoder, train_dataloader, criterion, criterion_event, epoch, indices0, indices1)
            logger.info("-----------------------------")
...
def train_epoch(Encoder, Decoder, train_dataloader, criterion, criterion_event, optimizer, epoch, total_step, args, indices0, indices1):
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        if args.dataset_name == 'ave_va':
            with torch.no_grad():# Freeze Encoder
                video_vq, video_vq_2, _, _ = Encoder.Video_VQ_Encoder(visual_feature)
            ...
            with torch.no_grad():# Freeze Encoder
                audio_vq, audio_vq_2, _, _ = Encoder.Audio_VQ_Encoder(audio_feature)
...
@torch.no_grad()
def validate_epoch(Encoder,Decoder, val_dataloader, criterion, criterion_event, epoch, indices0, indices1, eval_only=False):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
